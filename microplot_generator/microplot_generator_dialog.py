...
class MicroplotGeneratorDialog(QDialog, FORM_CLASS):
    ...
    def on_point_selected(self, point, point_number):
        getattr(self, f'point{point_number}_x').setText(str(point.x()))
        getattr(self, f'point{point_number}_y').setText(str(point.y()))
    ...
    def generate_grid(self, corner_points, rows, cols, plot_length, plot_width, output_file, crs, id_format, start_numbering_from, initial_id, fielddata_file_path, fielddata_plotid_col, fieldmap_file_path):
        ...
        source_crs = crs
        # We use EPSG:5347 for calculations as it is a projected CRS in meters
        dest_crs = QgsCoordinateReferenceSystem("EPSG:5347")

        transform = QgsCoordinateTransform(source_crs, dest_crs, QgsProject.instance())
        
        transformed_corner_points = [transform.transform(p) for p in corner_points]

        output_layer = QgsVectorLayer(f"Polygon?crs={dest_crs.authid()}", "Microplot Grid", "memory")
        ...
        if not current_plot_ids:
            ...
            if start_numbering_from in [0, 1]:  # Bottom Left, Bottom Right
                numbering_row_order.reverse()

            for row_iteration_index, r_visual in enumerate(numbering_row_order):
                numbering_col_order = list(range(cols))

                # Determine starting direction for the row
                starts_rtl = start_numbering_from in [1, 3]  # Bottom Right, Top Right

                if id_format == 1:  # Serpentine
                    if row_iteration_index % 2 != 0:
                        # Reverse direction for odd-numbered processing rows
                        starts_rtl = not starts_rtl
        ...
        if plot_length > 0 and plot_width > 0: # Lógica para parcelas de tamaño fijo (cálculo directo)
            p1, p2, p3, p4 = transformed_corner_points[0], transformed_corner_points[1], transformed_corner_points[2], transformed_corner_points[3]
